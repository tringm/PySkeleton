...
def execute_parser(parser):
    ...
    test_dir = project_root_path().joinpath(args.testDirPath)
    if not test_dir.exists():
        parser.error(f"Test dir {test_dir} does not exist")

    metric_f = args.metricsLogPath if args.metricsLogPath else str(datetime.now())
    metric_log_path = project_root_path().joinpath(metric_f)
    if not metric_log_path.parent.exists():
        parser.error(f"Metric log file director {metric_log_path.parent} does not exist")
...
def discover_test_suites(starting_dir: Path):
    ...
    for d in suites_dir:
        ...
        if tests.countTestCases() > 0:
            test_suites[str(d.relative_to(project_root_path()))] = tests
    return test_suites


def discover_test_methods(starting_dir: Path):
    relative_from_root = str(starting_dir.relative_to(project_root_path())).replace('/', '.')
